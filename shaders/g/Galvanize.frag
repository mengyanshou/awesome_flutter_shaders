...
// 全局变量改为在 mainImage 中初始化以防止状态污染
// 修复循环计数器类型和变量初始化
// Add required include directives for Flutter/Skia compatibility
// Global variables now initialized in mainImage to prevent state pollution
// Fixed loop counter types and variable initialization
