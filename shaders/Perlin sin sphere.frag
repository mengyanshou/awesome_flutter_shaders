...
float perlin3(vec3 p)
{
	vec3 o = floor(p * .5) + .5,
		 f = fract(p * .5),
		 t = f * f * f * (f * (f * 6. - 15.) + 10.);
	return .5 + mix(mix(mix(G(0, 0, 0), G(1, 0, 0), t.x),
						mix(G(0, 1, 0), G(1, 1, 0), t.x),
						t.y),
					mix(mix(G(0, 0, 1), G(1, 0, 1), t.x),
						mix(G(0, 1, 1), G(1, 1, 1), t.x), t.y),
					t.z);
}
...
float n(vec2 v)
{
	...
	return mix(mix(H(v1), H(v1 + vec2(0, 1)), v2.y),
			   mix(H(v1 + vec2(1, 0)), H(v1 + vec2(1)), v2.y), v2.x);
}
...
vec3 skyColor(vec3 p)
{
	...
	float d = length(p);
	return mix(vec3(.15, .3, .6) + v,
			   vec3(.2, .5, 1) + v * 12. / max(d, 20.),
			   min(d * .1, 1.));
}
...
{
	float y = abs(rd.y),
		  ds = clamp(dot(vec3(-1, 1, -1), rd) * .57735, 0., 1.);
	vec3 p = rd * 5. / y;
	return (rd.y > 0.
				? skyColor(p) + .3 * pow(1. - y, 3.) * .7
				: mix(floorColor(2. * p), vec3(.5, .7, 1), max(1. - sqrt(y) * 3., 0.))) +
...
vec4 distortHeighByNormal(vec3 uv)
{
	...
	{
		vec3 n = N(uv),
			 s = sin(vec3(perlin3(uv - d * n)) * 68.442 - iTime * 5.) * .868; // input_tex3d(uv - d * n)
...
vec4 O(vec3 uv, int w)
{
	...
	return vec4(w > 0 ? clamp(B.xyz * .882 + .636, 0., 1.) : vec3(0),
				B.w / 1.98882);
}
...
vec3 normal(vec3 p)
{
	...
	float v1 = O(p + e.xyy, 0).w,
		  v2 = O(p + e.yyx, 0).w,
		  v3 = O(p + e.yxy, 0).w,
		  v4 = O(p + e.xxx, 0).w;
	return N(vec3(v4 + v1 - v3 - v2,
				  v3 + v4 - v1 - v2,
				  v2 + v4 - v3 - v1));
}
...
{
	dS++;
	for (int i; ++i < 500 && d < 50. && abs(dS) > 1e-4;)
		d += dS = O(p = ro + rd * d, 0).w;
}
...
vec3 raymarch(vec2 uv)
{
	float d, dS;
	uv -= .5;
	vec3 cam = vec3(sin(iTime * .2) * 5., sin(iTime * .13) + 2., 5),
		 ray = N(-cam),
		 cX = N(cross(vec3(0, 1, 0), ray)),
		 rd = N(ray * 1.5 + cX * uv.x + cross(cX, ray) * uv.y),
		 p;
	march(d, p, dS, cam, rd);
	vec3 color = d < 50.
					 ? env(equirectangularMap(N(reflect(rd, -normal(p))).xzy)).xyz * mix(vec3(1), O(p, 1).xyz, .9)
					 : env(equirectangularMap(rd.xzy)).xyz;
