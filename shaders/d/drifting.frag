...
vec3 normal(vec2 p, float e, float depth) {
    ...
    return normalize(cross(a - vec3(p.x - e, calculate_wave_at_p(p.xy - ex.xy, RaymarchNormalSteps) * depth, p.y), a - vec3(p.x, calculate_wave_at_p(p.xy + ex.yx, RaymarchNormalSteps) * depth, p.y + e)));
} 
// -------------------- March ----------------------
...
vec3 render(vec2 uv) {
    ...
    vec3 ro = BobberAnimation ? vec3(iTime * 0.1, mix(CameraHeight, CameraHeight - WaterDepth, x), 1.0) : vec3(iTime * 0.1, CameraHeight, 1.0);

    mat3 rot = rotate_angle_axis(vec3(1.0, 0.0, 0.0), -0.18);
    vec3 rd = rot * normalize(vec3(uv, fov_rads / -2.5));
    vec3 lp = iMouse.z > 0.0 || !SunAnimation ? rot * normalize(vec3(mouse.xy, fov_rads / -2.5)) : rot * normalize(vec3(sin(iTime * 0.1) * 0.9, cos(iTime * 0.1) * 0.9, fov_rads / -2.5));
