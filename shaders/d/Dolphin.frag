...
vec2 sdDolphin(vec3 p) {
	...
	mat3 ms = mat3(d3.z / k, -d3.x * d3.y / k, d3.x, 0.0, k, d3.y, -d3.x / k, -d3.y * d3.z / k, d3.z);
	vec3 ps = p - p3;
	ps = ms * ps;
	ps.z -= 0.1;
	...
	ms = mat3(d1.z / k, -d1.x * d1.y / k, d1.x, 0.0, k, d1.y, -d1.x / k, -d1.y * d1.z / k, d1.z);
	ps = p - p1;
	ps = ms * ps;
	ps.x = abs(ps.x);
	float l = ps.x;
...
vec2 sdWaterCheap(vec3 p) {
	vec2 q = 0.1 * p.xz;
	float f = 0.0;
	f += 0.50000 * almostAbs(noise(q));
	q = m2 * q * 2.02;
	q -= 0.1 * iTime;
	f += 0.25000 * almostAbs(noise(q));
	q = m2 * q * 2.03;
	q += 0.2 * iTime;
	f += 0.12500 * almostAbs(noise(q));
	q = m2 * q * 2.01;
	q -= 0.4 * iTime;
	f += 0.06250 * almostAbs(noise(q));
	q = m2 * q * 2.02;
	q += 1.0 * iTime;
...
vec3 calcNormalFish(in vec3 pos) {
	...
	return normalize(vec3(sdDolphin(pos + eps.xyy).x - v, sdDolphin(pos + eps.yxy).x - v, sdDolphin(pos + eps.yyx).x - v));
#else
...
vec3 doLighting(in vec3 pos, in vec3 nor, in vec3 rd, float glossy, float glossy2, float shadows, in vec3 col, float occ) {
	...
	float shr = 1.0;
	if(shadows > 0.0)
		shr = softshadow(pos + 0.01 * nor, normalize(ref + vec3(0.0, 1.0, 0.0)), 0.0005, 8.0);
...
vec3 normalMap(in vec2 pos) {
	...
	return normalize(vec3(v - texture(iChannel3, 0.03 * pos + vec2(1.0 / 1024.0, 0.0)).x, 1.0 / 16.0, v - texture(iChannel3, 0.03 * pos + vec2(0.0, 1.0 / 1024.0)).x));
}
