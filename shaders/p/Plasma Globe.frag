...
// 迁移日志：
// - 初始化局部变量以避免未定义行为。
// - 将 for 循环计数器从 float 改为 int。
// - 替换 textureLod 为 texture 以兼容 SkSL。
// - 为 log() 和除法添加保护，防止出现非数值结果。
// --- Migrate Log (EN) ---
// - Initialize local variables to avoid undefined behavior.
// - Change for loop counters from float to int.
// - Replace textureLod with texture for SkSL compatibility.
// - Add protection for log() and division to prevent non-numeric results.
...
float segm( vec3 p, vec3 a, vec3 b)
{
    vec3 pa = p - a;
	vec3 ba = b - a;
	float h = clamp( dot(pa,ba)/max(dot(ba,ba), 1e-6), 0.0, 1. );
...
vec3 vmarch(in vec3 ro, in vec3 rd, in float j, in vec3 orig)
{
    ...
    for( int i=0; i<VOLUMETRIC_STEPS; i++ )
    {
        ...
        sum += abs(col)*5. * (1.2-noise(lp*2.+j*13.+time*5.)*1.1) / (log(max(distance(p,orig),2.001)-2.)+.75);
    }
    return sum;
}
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    #if 1
    for (int j = 1; j < int(NUM_RAYS) + 1; j++)
    {
        ro = bro;
        rd = brd;
        mat2 mm = mm2((time*0.1+((float(j)+1.)*5.1))*float(j)*0.25);
        ro.xy *= mm;rd.xy *= mm;
        ro.xz *= mm;rd.xz *= mm;
    ...
    if (sph.x > 0.)
    {
        ...
        float nz = (-log(max(abs(flow(rf*1.2,time)-.01), 1e-6)));
        float nz2 = (-log(max(abs(flow(rf2*1.2,-time)-.01), 1e-6)));
