...
const int[] font = int[](7 + 5 * 16 + 5 * 256 + 5 * 4096 + 7 * 65536, 2 + 2 * 16 + 2 * 256 + 2 * 4096 + 2 * 65536, 7 + 1 * 16 + 7 * 256 + 4 * 4096 + 7 * 65536, 7 + 4 * 16 + 7 * 256 + 4 * 4096 + 7 * 65536, 4 + 7 * 16 + 5 * 256 + 1 * 4096 + 1 * 65536, 7 + 4 * 16 + 7 * 256 + 1 * 4096 + 7 * 65536, 7 + 5 * 16 + 7 * 256 + 1 * 4096 + 7 * 65536, 4 + 4 * 16 + 4 * 256 + 4 * 4096 + 7 * 65536, 7 + 5 * 16 + 7 * 256 + 5 * 4096 + 7 * 65536, 7 + 4 * 16 + 7 * 256 + 5 * 4096 + 7 * 65536);
...
vec3 drawMap(vec3 col, in vec2 fragCoord) {
    ...
    if(q.x >= 0.0 && q.x <= 27.0) {
        ...
        // walls
            ...
            vec2 wmi = vec2(texture(iChannel0, (q - vec2(1.0, 0.0) + 0.5) / iResolution.xy).x, texture(iChannel0, (q - vec2(0.0, 1.0) + 0.5) / iResolution.xy).x);
            vec2 wma = vec2(texture(iChannel0, (q + vec2(1.0, 0.0) + 0.5) / iResolution.xy).x, texture(iChannel0, (q + vec2(0.0, 1.0) + 0.5) / iResolution.xy).x);
...
vec3 drawPacman(vec3 col, in vec2 fragCoord, in vec4 pacmanPos, in vec3 pacmanMovDirNex) {
    ...
    if(pacmanMovDirNex.y < 1.5) {
        q = q.xy * vec2(-1.0, 1.0);
    } else if(pacmanMovDirNex.y < 2.5) {
        q = q.xy;
    } else if(pacmanMovDirNex.y < 3.5) {
        q = q.yx * vec2(-1.0, 1.0);
    } else {
        q = q.yx;
    }
