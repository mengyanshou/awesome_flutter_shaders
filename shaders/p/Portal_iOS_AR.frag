...
#include <Portal_iOS_AR_Common.frag>
uniform sampler2D iChannel0;
uniform sampler2D iChannel1;
uniform sampler2D iChannel2;
uniform sampler2D iChannel3;
// Portal_iOS_AR. Created by Reinder Nijhoff 2018
...
float map(in vec3 p) {
    ...
    vec2 pm = mod(p.xz + vec2(PILLAR_SPACING * .5), vec2(PILLAR_SPACING)) -
        vec2(PILLAR_SPACING * .5);
