...
float Voronoi(in vec2 p){
	...
	vec2 g = floor(p), o = vec2(0.0); p -= g;
	
	vec3 d = vec3(1.0); // 1.4, etc. "d.z" holds the distance comparison value.
...
float heightMap(vec3 p){
    ...
    id = 0;
    float c = Voronoi(p.xy*4.0); // The fiery bit.
...
vec3 nr(vec3 p, inout float edge) { 
	...
	float d = m(p)*2.0;	// The hit point itself - Doubled to cut down on calculations. See below.
...
vec3 eMap(vec3 rd, vec3 sn){
    ...
    float c = n3D(rd)*0.57 + n3D(rd*2.0)*0.28 + n3D(rd*4.0)*0.15; // Noise value.
    c = smoothstep(0.5, 1.0, c); // Darken and add contast for more of a spotlight look.
    
    //vec3 col = vec3(c, c*c, c*c*c*c).zyx; // Simple, warm coloring.
    vec3 col = vec3(min(c*1.5, 1.0), pow(c, 2.5), pow(c, 12.0)).zyx; // More color.
...
void mainImage(out vec4 c, vec2 u){
    ...
    vec3 r = normalize(vec3(u - iResolution.xy*0.5, iResolution.y)), 
         o = vec3(0.0), l = o + vec3(0.0, 0.0, -1.0);
   
    // Rotate the canvas. Note that sine and cosine are kind of rolled into one.
    vec2 a = sin(vec2(1.570796, 0.0) + iTime/8.0); // Fabrice's observation.
    ...
    float edge = 0.0; // Edge value - to be passed into the normal.
    ...
    if(t<FAR){
        ...
        //
        c.x = dot(c.xyz, vec3(0.299, 0.587, 0.114)); // Grayscale.
        if (id==0) c.xyz *= vec3(min(c.x*1.5, 1.0), pow(c.x, 5.0), pow(c.x, 24.0))*2.0;
        else c.xyz *= 0.1;
        ...
        float df = max(dot(l, n), 0.0); // Diffuse.
        float sp = pow(max(dot(reflect(-l, n), -r), 0.0), 32.0); // Specular.
        
        if(id == 1) sp *= sp; // Increase specularity on the dark lattice.
        
		// Applying some diffuse and specular lighting to the surface.
        c.xyz = c.xyz*(df + 0.75) + vec3(1.0, 0.97, 0.92)*sp + vec3(0.5, 0.7, 1.0)*pow(sp, 32.0);
        ...
        c.xyz *= 1.0 - edge*0.8; // Darker edges.
    ...
    c = vec4(sqrt(clamp(c.xyz, 0.0, 1.0)), 1.0);
