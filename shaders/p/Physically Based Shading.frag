...
const vec3 BASE_COLORS[6] = vec3[6](vec3(0.74), vec3(0.51, 0.72, 0.81), vec3(0.66, .85, .42), vec3(0.87, 0.53, 0.66), vec3(0.51, 0.46, 0.74), vec3(0.78, 0.71, 0.45));
...
vec3 SceneNormal(in vec3 pos, mat3 localToWorld) {
	...
	vec3 nor = vec3(Scene(pos + eps.xyy, localToWorld) - Scene(pos - eps.xyy, localToWorld), Scene(pos + eps.yxy, localToWorld) - Scene(pos - eps.yxy, localToWorld), Scene(pos + eps.yyx, localToWorld) - Scene(pos - eps.yyx, localToWorld));
...
const SHCoefficients SH_STPETER = SHCoefficients(vec3(0.3623915, 0.2624130, 0.2326261), vec3(0.1759131, 0.1436266, 0.1260569), vec3(-0.0247311, -0.0101254, -0.0010745), vec3(0.0346500, 0.0223184, 0.0101350), vec3(0.0198140, 0.0144073, 0.0043987), vec3(-0.0469596, -0.0254485, -0.0117786), vec3(-0.0898667, -0.0760911, -0.0740964), vec3(0.0050194, 0.0038841, 0.0001374), vec3(-0.0818750, -0.0321501, 0.0033399));
...
void DrawScene(inout vec3 color, vec2 p, in AppState s) {
	...
	float a = -iTime * .5;
	mat3 rot = mat3(vec3(cos(a), 0., -sin(a)), vec3(0., 1., 0.), vec3(sin(a), 0., cos(a)));

	p -= vec2(-20., 10.);
	p *= .011;

	float yaw = 2.7 - s.objRot;
	mat3 rotZ = mat3(vec3(cos(yaw), 0.0, -sin(yaw)), vec3(0.0, 1.0, 0.0), vec3(sin(yaw), 0.0, cos(yaw)));

	float phi = -0.1;
	mat3 rotY = mat3(vec3(1.0, 0.0, 0.0), vec3(0.0, cos(phi), sin(phi)), vec3(0.0, -sin(phi), cos(phi)));
	...
	if(t > 0.0) {
		...
		if(s.menuId == MENU_GEOMETRY) {
			color = vec3(lightV) * (4.0 * ndotv * ndotl);
		}
