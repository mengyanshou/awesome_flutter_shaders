...
void renderBox(int ci) {
    vec4 q =//vec4(0,0,0,1),
    getCubeQuat(ci), qi = vec4(-q.xyz, q.w);
    vec3 pos = getCubePos(ci);
    vec3 b = size(ci);
    int sh = shape(ci);

#ifndef MPR       
    vec3 ro = rotate(qi, rayPos - pos), rd = rotate(qi, rayDir);
    vec2 t;
    if(sh == 0)
        t = iBox(ro, rd, b);
    else if(sh == 1)
        ...
        t = iCylinderVertical(ro, rd, b.y, b.x);
...
vec3 getDiffuse() {
    vec3 difColor;
    if(minDistMaterial == 0.)
        difColor = texture(iChannel0, rayPos.zx / 8.0).xyz; // floor
    else if(minDistMaterial == 1.)
        difColor = texture(iChannel1, minDistUv / 2.).xyz; // floor
    else if(minDistMaterial >= 2.)
        difColor = texture(iChannel2, minDistUv / 2.).xyz;
    else
        difColor = vec3(1, 0, 0);

    if(minDistMaterial == 3.)
        difColor = difColor.zyx;
    if(minDistMaterial == 4.)
        difColor = difColor.yxz;
    if(minDistMaterial == 5.)
        difColor = difColor.xyz;
    if(minDistMaterial == 6.)
        difColor = difColor.zxy;
    if(minDistMaterial == 7.)
        difColor = difColor.yzx;
    return difColor;
}
...
vec3 getRayDir(vec3 ro, vec3 lookAt, vec2 uv) {
    vec3 f = normalize(lookAt - ro), r = normalize(cross(vec3(0, 1, 0), f));
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(iMouse.z > 0.) {
        a = iMouse.x / iResolution.x * 4. - 2.;
        d *= iMouse.y / iResolution.y + .8;
    }
