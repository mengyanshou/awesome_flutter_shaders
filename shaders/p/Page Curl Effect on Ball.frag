...
////////BASE OBJECTS///////////////////////
float obox(vec3 p, vec3 b) {
    return length(max(abs(p) - b, 0.0));
}
float osphere(vec3 p, float r) {
    return length(p) - r;
}
////////MAP////////////////////////////////
vec4 map(vec3 p) {
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    if(d < maxd) {
        ...
        vec3 cubeRefl = texture(iChannel0, reflRay.xy).rgb * refl_i;
        vec3 cubeRefr = texture(iChannel0, refrRay.xy).rgb * refr_i;
