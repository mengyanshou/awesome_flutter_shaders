// --- Migrate Log ---
// - 移除 mainVR，用 mainImage 替代
// - 替换 texelFetch 为 texture
// - 初始化局部变量
// - 添加 common header 和 iChannel 声明
// - 在 iPlane 和 fogAmount 计算中添加除零保护
// - 固定 fbm 为唯一使用的 4 次迭代，并移除 sampler 函数参数；保留全局 sampler 的原生 texture() 采样语义
//
// - Removed mainVR, replaced with mainImage
// - Replaced texelFetch with texture
// - Initialized local variables
// - Added common header and iChannel declarations
// - Added division-by-zero protection in iPlane and fogAmount calculation
// - Fixed fbm to its only four-iteration use and removed sampler function parameters; preserve native texture() sampling through global samplers
...
vec4 tex3D( in vec3 p, in vec3 n ) {
    p.xz = mat2(0.8,-0.6,0.6,0.8) * p.xz + .5;
    
	vec4 x = texture(iChannel2, p.yz);
	vec4 y = texture(iChannel2, p.zx);
	vec4 z = texture(iChannel2, p.xy);
...
vec3 render( in vec3 ro, in vec3 rd, in vec2 uv, bool inside ) {
    float portalAlpha = 0.;
    vec3 fogColor = vec3(0.1,0.3,.5) + rd * .1;
    float tmin = 0.01;
    const float tmax = 21.;
    
    vec3 portalColor = texture(iChannel3, uv).rgb * 1.25;
    // Use mipmap level 9 to get an average environment color from the webcam texture
    // used for lighting.
    // SkSL does not support texelFetch with LOD, use texture at center as an approximation
    vec3 lightColor = pow(.25 + .75 * texture(iChannel3, vec2(0.5)).rgb, vec3(2.2)) * 3.;
      
    // portal intersection
    float portalDist = 0.0;
    portalDist = iPlane( ro, rd, vec4(0,0,1,-dot(PORTAL_POS,vec3(0,0,1))));
    if (portalDist < 0.) {
        portalDist = 5e5;
    } else {
        ...
        float time = iTime * .15;
        float scale = 6.;
        vec2 offset = vec2(fbm(p.xy * scale + time), fbm(p.yx * scale - time)) -.5;
        p.xy += (fbm(offset * scale + time) - .5) * .2;
    ...
    float fogAmount = 0.0;
    if (abs(rd.y) > 1e-6) {
        fogAmount = clamp(C * exp(-ro.y*B) * (1.-exp( -t*rd.y*B))/rd.y, 0., 4.);
    }
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    vec3 col = render( ro, rd, fragCoord.xy/iResolution.xy, false);
