// --- Migrate Log ---
// - 在 light 函数中为除法添加保护，防止除以零。
// - 添加 include 和迁移日志，初始化循环变量和条件判断
// --- Migrate Log (EN) ---
// - Add protection for division in the light function to prevent division by zero.
// - Added include and migration log, initialized loop variables and conditions
...
vec3 light(vec3 p,vec3 view) {
    ...
    float emissive = clamp(0.001 / max(abs((mod(abs(p.y * sx + p.x * sy) + iTime * sign(sx - 0.5) * 0.4, k) - 0.5 * k)), 1e-6), 0.0, 1.0);
