...
mat4 transposeM4(in mat4 m) {
    ...
    mat4 t = mat4(vec4(r0.x, r1.x, r2.x, r3.x), vec4(r0.y, r1.y, r2.y, r3.y), vec4(r0.z, r1.z, r2.z, r3.z), vec4(r0.w, r1.w, r2.w, r3.w));
    return t;
}
...
#define RADIUS 0.666
    ...
    d = sdCapsule(p, vec3(0.0, 0.68, -0.7), vec3(0.02, 0.07, 0.8), 0.27);
#endif

#if LESSON == 12 // Composite: Capsule (sin + Box)
    p = q;
    //p.y += sin(p.y*PI/2.0)*0.25; // original *1.7)*0.3
    d = sdBox(vec3(0.0, 0.68, -0.7) + p, vec3(0.02, 0.07, 0.8), 0.27);
#endif
...
*/
    ...
    float a = (length(p.xz) - 1.0 - p.y * .15) * .85, a2 = (length(p.xz) - 0.9 - p.y * .15) * .85;
    a = max(abs(p.y) - 1.0, a);
    ...
    d = opU(opU(d1, d2), d3);
#else
    d = opI(opU(d1, d2), opU(opI(d1, d2), d3));
#endif
#endif // 333
...
sMat getMaterial(sHit hitInfo) {
    ...
    } else if(hitInfo.oid.x == MATERIAL_2) {
        mat.frs = 0.0;
        mat.smt = 1.0;
        mat.trs = 0.0;
        mat.fri = 0.0;
...
vec3 getBackground(vec3 rd) {
    ...
    return mix(cc, tc, f);
#else
    // 修改这一行，使用texture2D并正确处理返回值
    vec4 texColor = texture(iChannel0, rd.xy);
    return texColor.rgb; // 使用.rgb替代.xyz
#endif
}
...
vec3 normal(vec3 p, float fts) {
    ...
    return normalize(vec3(e.xyy * DE(p + e.xyy, fts).x +
...
vec3 getRayColor(sRay ray) {
    ...
    if(i.oid.x < 0.5) {
        ...
        if(m.trs > 0.0)
            s.tra = getTransparency(ray, i, s, m);
