...
vec3 Render(vec2 uvScreen, int idx, int maxSamples, bool isDisplaced, float jpegDamage, out float blend) {       
    ...
    for(int iterIdx = 0; iterIdx < kMaxIterations; ++iterIdx) {
        ...
        if(hexHash % 2u == 0u) {
            ...
            if(dist > 1.0 - 0.02 * float(subdiv)) {
                invert = invert ^ 1;
            } else if(fract(20. / float(subdiv) * hashSum) < 0.5) {
                invert = invert ^ 1;
            }
            if(iterIdx == 0)
                break;
...
void mainImage(out vec4 rgba, in vec2 xy) {
    ...
    if(isDisplaced) {
        ...
        if(fract(rgb.x) > jpegDamage)
            rgb.x += 1.0;
        if(fract(rgb.y) > jpegDamage)
            rgb.y += 1.0;
        if(fract(rgb.z) > jpegDamage)
            rgb.z += 1.0;
