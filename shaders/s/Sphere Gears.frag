...
//
// --- Migrate Log ---
// 替换 ZERO 宏为 int(min(iFrame,0.0)) 以避免 int/float 混用；用 floor/mod 替换位移/位与操作以兼容 SkSL；添加 common_header include 与声明 iChannel0
// --- Migrate Log (EN) ---
// Replaced ZERO macro with int(min(iFrame,0.0)) to avoid int/float mixing; replaced bit shifts and bitwise ops with floor/mod for SkSL; added common_header include and iChannel0 declaration
