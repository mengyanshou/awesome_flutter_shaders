// --- Migrate Log ---
// 添加迁移日志并引入 common_header；将整数字面值改为浮点以提高 SkSL 兼容性
// --- Migrate Log (EN) ---
// Added migration log and included common_header; changed integer literals to floats for SkSL compatibility
...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
	...
	if(i < tmax) {
		...
		col  = 0.1*vec3(0.0, 0.0, 1.0);
		col += 0.9*clamp(dot(key, nor), 0.0, 1.0)*vec3(1.0/max(1.0, i), 1.0, 1.0);
		col += 0.4*clamp(dot(-key, nor), 0.0, 1.0)*vec3(1.0/max(1.0, i), 1.0, 1.0);

		col *= material(pos);
	}

	col = mix(col, vec3(0.0), 1.0 - exp(-0.6*i));
