...
{
    return texture(iChannel3, (vec2(x, y) + 0.5) / iChannelResolution[3].xy);
}

vec4 loadD(int x, int y)
{
    return texture(iChannel2, (vec2(x, y) + 0.5) / iChannelResolution[2].xy);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    if (floor(fragCoord.x) < view_size)
    {
        ...
        #if BICUBIC_SHARPEN
            vec3 l = texture(iChannel2, (stc - fms3 + vec2( 3, 0)) / res).rgb;
            vec3 r = texture(iChannel2, (stc - fms3 + vec2(-3, 0)) / res).rgb;
            vec3 t = texture(iChannel2, (stc - fms3 + vec2( 0, 3)) / res).rgb;
            vec3 b = texture(iChannel2, (stc - fms3 + vec2( 0,-3)) / res).rgb;
    ...
    else
    {
        ...
        #if SRCNN_SHARPEN
        {
            float l = texture(iChannel2, (stc - vec2(view_size, 0.0) - fms3 + vec2( 3, 0)) / res).a;
            float r = texture(iChannel2, (stc - vec2(view_size, 0.0) - fms3 + vec2(-3, 0)) / res).a;
            float t = texture(iChannel2, (stc - vec2(view_size, 0.0) - fms3 + vec2( 0, 3)) / res).a;
            float b = texture(iChannel2, (stc - vec2(view_size, 0.0) - fms3 + vec2( 0,-3)) / res).a;
