...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    vec4 blurColor = texture(iChannel0, vec2(uv + shift*aber.x));
    blurColor.y =    texture(iChannel0, vec2(uv + shift*aber.y)).y;
    blurColor.z =    texture(iChannel0, vec2(uv + shift*aber.z)).z;

    vec4 sharpColor = texture(iChannel0, vec2(uv + shift*aber.x));
