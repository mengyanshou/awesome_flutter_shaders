// --- Migrate Log ---
// 添加 Flutter/SkSL 公共 include，声明缺失的采样器 iChannel0；
// 将 triNoise 中的浮点循环改为整型以兼容 SkSL（避免 float 作为循环计数器）
// --- Migrate Log (EN) ---
// Add Flutter/SkSL common include and declare missing sampler iChannel0;
// Replace float loop in triNoise with int loop for SkSL compatibility (avoid floats as loop counters)
