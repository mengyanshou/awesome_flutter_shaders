...
vec3 noised( in vec2 x )
{
    ...
    vec2 uv = vec2(p + vec2(0.5,0.5));
    uv = mod(uv, 256.0); // 替换位运算为取模运算
    float a = texture( iChannel0, uv/256.0 ).x;
    uv = vec2(p + vec2(1.5,0.5));
    uv = mod(uv, 256.0);
    float b = texture( iChannel0, uv/256.0 ).x;
    uv = vec2(p + vec2(0.5,1.5));
    uv = mod(uv, 256.0);
    float c = texture( iChannel0, uv/256.0 ).x;
    uv = vec2(p + vec2(1.5,1.5));
    uv = mod(uv, 256.0);
    float d = texture( iChannel0, uv/256.0 ).x;
#else    
    // texture version    
    vec2 p = floor(x);
