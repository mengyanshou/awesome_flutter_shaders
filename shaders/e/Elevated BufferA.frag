// --- Migrate Log ---
// 替换 texelFetch 为 SG_TEXELFETCH0，替换 texture(...) 为 SG_TEX0/SG_TEX1，以确保 wrap/filter 在 Flutter/Skia 下一致
// 添加并包含 common header 与 main_shadertoy.frag（未添加数值稳定性保护）
//
// Replaced texelFetch with SG_TEXELFETCH0 and texture(...) with SG_TEX0/SG_TEX1 so wrap/filter obey iChannel settings
// Added `#include <../common/common_header.frag>` and appended `#include <../common/main_shadertoy.frag>`
...
#if 0
    // texel fetch version (migrated to SG_TEXELFETCH0 macros)
    ivec2 p = ivec2(floor(x));
    float a = SG_TEXELFETCH0(p + ivec2(0,0)).x;
	float b = SG_TEXELFETCH0(p + ivec2(1,0)).x;
	float c = SG_TEXELFETCH0(p + ivec2(0,1)).x;
	float d = SG_TEXELFETCH0(p + ivec2(1,1)).x;
#else    
    // texture version    
    vec2 p = floor(x);
	float a = SG_TEX0(iChannel0, (p+vec2(0.5,0.5))/256.0).x;
	float b = SG_TEX0(iChannel0, (p+vec2(1.5,0.5))/256.0).x;
	float c = SG_TEX0(iChannel0, (p+vec2(0.5,1.5))/256.0).x;
	float d = SG_TEX0(iChannel0, (p+vec2(1.5,1.5))/256.0).x;
#endif
...
{
    float f = 0.0;
    f += 0.5000*SG_TEX0(iChannel0, p/256.0).x; p = m2*p*2.02;
    f += 0.2500*SG_TEX0(iChannel0, p/256.0).x; p = m2*p*2.03;
    f += 0.1250*SG_TEX0(iChannel0, p/256.0).x; p = m2*p*2.01;
    f += 0.0625*SG_TEX0(iChannel0, p/256.0).x;
    return f/0.9375;
}
...
vec4 render( in vec3 ro, in vec3 rd )
{
	...
	else
	{
        ...
        // rock
		float r = SG_TEX0(iChannel0, (7.0/SC)*pos.xz/256.0).x;
        col = (r*0.25+0.75)*0.9*mix( vec3(0.08,0.05,0.03), vec3(0.10,0.09,0.08), 
                                     SG_TEX0(iChannel0,0.00007*vec2(pos.x,pos.y*48.0)/SC).x );
