// --- Migrate Log ---
// 本次迁移修改:
// 初始化局部变量，改为 int 循环，初始化 out 参数
// change summary:
// Initialize local variables, change to int loop, initialize out parameters
// -------------------
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
	...
	if (intersectSphere(o, d, tmin, tmax))
	{
		...
		for (int i = 0; i < int(SLICES); i++)
		{
			...
			float density = START_DENSITY;
			for (int j = 0; j < 3; j++)
			{
				density += 0.5/abs((fbm(5.0*curpos*b+ANIMATION_SPEED*iTime/b)*2.0-1.0)/a);
				b *= 2.0;
				a /= 2.0;
			}

			// get gradient color depending on s
			col.rgb += gradient(s)*max(density*s,0.0);
		}
	}
