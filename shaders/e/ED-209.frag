// --- Migrate Log ---
// 添加必要的头文件包含，修复 AA 循环变量类型为 int 以兼容 GLSL 规范
// Added required header includes, fixed AA loop variables to int type for GLSL compliance
...
void mainImage(out vec4 fragColor, vec2 fragCoord) {
	...
	for (int dx = 0; dx <= 1; dx++) {
		for (int dy = 0; dy <= 1; dy++) {
			vec2 coord = fragCoord + vec2(float(dx), float(dy)) * .5;
#else
