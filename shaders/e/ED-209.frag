...
MarchData gunPod(vec3 p) {
	...
	pp.x = abs(p.x);
	pp.xy *= rot(.78525);
	float fs, bump = sign(sin(pp.z * 33.3)) * .003, d = sdBox(pp, vec3(.1 - bump, .38 - bump, .34)) - .02;
...
MarchData waist(vec3 p) {
	...
	p.y += .65;
	p.yz *= rot(-.2);
	float bump, d, legAngle = legWalkAngle(1.);
	p.xy *= rot(legAngle * .3);
	vec3 pp = p;
	pp.y += .3;
...
MarchData legs(vec3 p) {
	MarchData r;
	setBodyMaterial(r);
	float silver, legAngle = legWalkAngle(1.);
	p.z += .27;
...
MarchData room(vec3 p) {
	...
	p.x = abs(p.x);
	p.yz += vec2(.5, -3.4);
	float doorFrame, doorWidth, door, d, doorHole = sdBox(p, frameInner + vec3(0, 0, 1)), backWall = length(p.z - 8.);
...
vec3 applyLighting(vec3 p, vec3 rd, float d, MarchData data) {
	vec3 sunP = vec3(10, 10, -10), sunDir = normalize(sunP - p), n = calcNormal(p, d);

	// Primary light.
	float primary = max(0., dot(sunDir, n)), bounce = max(0., dot(-sunDir, n)) * .3, spe = pow(max(0., dot(rd, reflect(sunDir, n))), data.specPower) * 2., fre = smoothstep(.7, 1., 1. + dot(rd, n)), fog = exp(-length(p) * .05);

	primary /= pow(length(sunP - p) / 15., 3.0);
	if(data.specPower > 1e6)
		spe *= 0.;
...
void mainImage(out vec4 fragColor, vec2 fragCoord) {
	...
	// Camera.
	vec3 ro, lookAt, col;
	float startScene, endScene, dim, time = mod(iTime, 55.);
	if(time < 12.) {
		startScene = 0.;
		endScene = 12.;
		edWalk = 0.;
