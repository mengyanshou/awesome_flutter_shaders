// --- Migrate Log ---
// 初始化局部变量（camPos / camUp / camLookat）以避免未定义行为
// 用 int 循环替换 float antialiasing loop 以兼容 SkSL
// 添加并包含 common header 与底部 main_shadertoy.frag
//
// Initialize camera local variables to avoid undefined behavior
// Replace float antialiasing loop with an int loop for SkSL compatibility
// Add `#include <../common/common_header.frag>` and bottom `#include <../common/main_shadertoy.frag>`
...
vec3 RayTrace(in vec2 fragCoord )
{
    glow = 0.0;
	// -------------------------------- animate ---------------------------------------
	vec3 camPos = vec3(0.0), camUp = vec3(0.0), camLookat = vec3(0.0);
...
#ifdef NON_REALTIME_HQ_RENDER
    for (int si = 0; si < int(antialiasingSamples); si++)
    {
