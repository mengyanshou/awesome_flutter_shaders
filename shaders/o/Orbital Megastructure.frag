...
const uint niceColors[4] = uint[](matRGB | (76u << 24) | (67u << 16) | (8u << 8), matRGB | (76u << 24) | (10u << 16) | (4u << 8), matRGB | (50u << 24) | (96u << 16) | (86u << 8), matRGB | (10u << 24) | (7u << 16) | (3u << 8));
    ...
    if(randBigger.w < 0.97) {
        if(randBig.w > 0.15) {
            ...
            if(rand2.y > 0.5) {
                ...
                if(rand2.z > 0.5)
                    d = min(d, sdHexPrism((baseCenter - vec3(0.0, height, 0.0)).xzy, vec2(baseRad * 0.7, height2)) - 0.05);
            }
...
//            litf -= texture(iChannel2,vec2(pint)*0.01).x;
    ...
    if(randBigger.y > 0.6) {
        ...
        if(randBig.y > 0.5) {
            uint m = matPipe;
            if(randBig.x > 0.95)
                m = matYellow;// SetMatRGB(100u,52u,40u);
            matmin(dist, mat, d, m);
        }
    }
...
/*    vec3 pRot = RotateY(pos,ROT_SPEED*iTime);
    ...
    cyl.z = cyl.y;*/
        ...
        if((t > maxDepth) || (abs(dist) < smallVal))
            break;
    }
...
//            texColor = normal.xyz*0.5+0.5;// vec3(0.0) + rgbspec.aaa*4.0;
            ...
            if(length(pRot) > 7.0)
                texColor += rgbspec.aaa * 7.0 - 0.39;
            else
                texColor = max(vec3(0.33), texColor + rgbspec.aaa * 4.0);
