// --- Migrate Log ---
// 添加 common_header 以确保 uniforms (iResolution, iTime, iMouse) 可用。
// 避免修改全局 `sun_dir`（在原实现中使用 `sun_dir *= rot;`），改为计算局部 `sun` 并将其作为参数传入 `get_incident_light`，以避免全局状态修改与潜在 SkSL 兼容性问题（最小改动）。
// --- Migrate Log (EN) ---
// Insert common_header to provide uniforms (iResolution, iTime, iMouse).
// Avoid mutating global `sun_dir` (original used `sun_dir *= rot;`); compute a local `sun` and pass it into `get_incident_light` to prevent global state mutation and improve SkSL compatibility (minimal change).
// ----------------------------------------------------------------------------
...
vec3 get_incident_light(_in(ray_t) ray, vec3 sun)
{
...
void mainImage(_out(vec4) fragColor, vec2 fragCoord)
{
    ...
    if (u_mouse.z < 0.1) {
        ...
        col = get_incident_light(ray, sun);
    } else {
