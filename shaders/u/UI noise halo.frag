...
// --- Migrate Log ---
// 添加 common_header 引入；初始化 draw() 内部局部变量以避免未定义行为；显式设置输出 alpha
// --- Migrate Log (EN) ---
// Added common_header include; initialized local variables inside draw() to avoid undefined behavior; set output alpha explicitly
...
void draw( out vec4 _FragColor, in vec2 vUv )
{
    ...
    float len = length(uv);
    float v0 = 0.0, v1 = 0.0, v2 = 0.0, v3 = 0.0, cl = 0.0;
    float r0 = 0.0, d0 = 0.0, n0 = 0.0;
    float r = 0.0, d = 0.0;
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    fragColor = vec4( mix(bg, col.rgb, col.a), 1.0 ); // normal blend with explicit alpha
}
