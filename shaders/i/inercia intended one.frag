...
vec3 pcg3df(vec3 s){
  // Float-only hash to avoid unsigned bit operations.
  vec3 p = fract(s * vec3(0.1031, 0.11369, 0.13787));
...
{
  vec2 uv = vec2(I.x / iResolution.x, I.y / iResolution.y);
  vec2 p=uv*2.-1.;
  ...
  if(march.isect.x<1E-2){
    ...
    if(mtl==0.){
      ...
      float meterSeed = march.grid.h.z * 12.345;
      float slowOsc = 0.5 + 0.5 * sin(iTime * 0.6 + meterSeed);
      float envelope = smoothstep(0.0, 1.0, fract(iTime * 0.08 + meterSeed));
      float lv = clamp(slowOsc * (0.6 + 0.4 * envelope), 0.0, 1.0);
