...
// diagonal matrix
mat4 diag(vec4 vec) {
    return mat4(vec.x, 0, 0, 0, 0, vec.y, 0, 0, 0, 0, vec.z, 0, 0, 0, 0, vec.w);
}
...
mat4 tetrad(vec4 x, vec4 time, vec4 aim, vec4 vert) {
    ...
    vec4 E2 = unit(inverse(g) * vec4(dot(E0.yzw, cross(E1.yzw, E3.yzw)), -dot(E0.zwx, cross(E1.zwx, E3.zwx)), dot(E0.wxy, cross(E1.wxy, E3.wxy)), -dot(E0.xyz, cross(E1.xyz, E3.xyz))), g);
    mat4 tetrad;
    tetrad[0] = E0;
    tetrad[1] = E1;
    tetrad[2] = E2;
