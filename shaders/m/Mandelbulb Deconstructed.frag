// --- Migrate Log ---
// 初始化所有局部变量以避免未定义行为
// 保护 log(r)*r/derivative 以防 r 为 0
// 保留原始宏定义，不重新定义
// --- Migrate Log (EN) ---
// Initialize all local variables to avoid undefined behavior
// Protect log(r)*r/derivative against r=0
// Preserve original macro definitions without redefining
...
float distanceToSurface(Point3 P, out float AO) {
	...
	for (int i = 0; i < ITERATIONS; ++i) {
		...
		if (r > 2.0) {	
			...
			return min(length(P) - internalBoundingRadius, 0.5 * log(safeR) * safeR / derivative);
		} else {		
