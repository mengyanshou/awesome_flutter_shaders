...
// 文件 / File: Monterey wannabe.frag
// 
// 关键修改 / Key Changes:
// 1. 移除 C 风格浮点后缀 (0.6f → 0.6) / Remove C-style float suffixes
...
} // 修改 1 / Change 1: 移除 C 风格浮点后缀 (0.6f → 0.6 等)
...
void mainImage( out vec4 fragColor, in vec2 fragCoord) {
  ...
  vec3 col = vec3(0.0);
  col = effect(p, q);
  col += 2.0*smoothstep(4.0, 0.0, iTime+length(p-vec2(0.0, 1.0)));
  col = aces_approx(col);
  col = sRGB(col);
  fragColor = vec4(col, 1.0);
} // 修改 2 / Change 2: 标准化浮点值
