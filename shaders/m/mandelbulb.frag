// --- Migrate Log ---
// 初始化所有局部变量以避免未定义行为（r, t, d, pd, step, error）
// 保护 log(r)*r/dr 防止 r=0
// 确保 int 循环计数器
// --- Migrate Log (EN) ---
// Initialize all local variables to avoid undefined behavior (r, t, d, pd, step, error)
// Protect log(r)*r/dr against r=0
// Ensure int loop counters
...
vec3 intersect( in vec3 ro, in vec3 rd )
{
    ...
    float res_t = 0.0;
    float res_d = 1000.0;
    vec3 c = vec3(0.0), res_c = vec3(0.0);
