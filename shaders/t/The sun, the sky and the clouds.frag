// --- Migrate Log ---
// 添加 common_header include 并补充缺失的采样器声明；替换带 LOD/bias 的 texture 调用为简单采样；把使用 float 循环计数器的 for 改为整数计数器并显式转换以兼容 SkSL；在文件末尾添加 main include。
// --- Migrate Log (EN) ---
// Added common_header include and missing sampler declarations; replaced textureLod/texture(...,lod) with simple texture(); converted float for-loops to int counters with explicit casts for SkSL compatibility; appended main include at file end.
...
vec4 aurora(vec3 ro, vec3 rd)
{
    ...
    for (int ii = 0; ii < 5; ii++) {
        ...
        float pt = ((0.8 + pow((i * mt), 1.2) * 0.001) - rd.y) / (rd.y * 2. + 0.4);
        pt -= of;
        vec3 bpos = (ro) + pt * rd;
        vec2 p = bpos.zx;
        //vec2 p = rd.zx;
        ...
        col2.rgb = (sin(1. - vec3(2.15, -0.5, 1.2) + (i * mt) * 0.053) * (0.5 * mt)) * rzt;
...
float noise(in vec2 v) {
    return texture(iChannel0, (v + 0.5) / 256.0).r;
}
