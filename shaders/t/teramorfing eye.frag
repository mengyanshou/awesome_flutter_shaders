/*originals from https://www.shadertoy.com/view/XfyXRV*/
...
const vec4 hsv2rgb_K = vec4(0.5, 0.5, 0.5, 0.5);
vec3 hsv2rgb(vec3 c) {
...
vec3 aces_approx(vec3 v) {
  v = max(v, 0.0);
  v.xz *= mat2(cos(iTime * 0.1), sin(iTime * 0.1), -sin(iTime * 0.1), cos(iTime * 0.1));
  v *= 0.6;
  float a = 2.51;
  float b = 0.03;
  float c = 2.43;
...
float box(vec2 p, vec2 b) {
  p.yx *= mat2(cos(iTime * 1.1), sin(iTime * 1.1), -sin(iTime * 1.1), cos(iTime * 1.1));
  vec2 d = abs(p) - b;
...
float poly_plane(vec3 pos) {
  pos.xy *= mat2(cos(iTime * 1.1), sin(iTime * 1.1), -sin(iTime * 1.1), cos(iTime * 1.1));
...
float poly_corner(vec3 pos) {
  pos.xz *= mat2(cos(iTime * 1.1), sin(iTime * 1.1), -sin(iTime * 1.1), cos(iTime * 1.1));
...
vec3 shape(vec3 pos) {
  ...
  pos *= g_rot;
  pos.xy *= mat2(cos(iTime * 1.1), sin(iTime * 1.1), -sin(iTime * 1.1), cos(iTime * 1.1));
  pos.zy *= mat2(cos(iTime * 1.1), sin(iTime * 1.1), -sin(iTime * 1.1), cos(iTime * 1.1));
  pos /= poly_zoom;
  poly_fold(pos);
  pos -= poly_p;
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
  ...
  fragColor = vec4(col * vec3(0.5, 0.5, 0.5) * 3., 1.0);
}
