...
void mainImage(out vec4 O, vec2 C) {
    ...
    vec2 R = iResolution.xy, u = (U + U - R) / R.y * 1.2, m = (iMouse.xy * 2. - R) / R.y;

    float t = iTime / 60., l = 15.,  // loop size
    ...
    for(; i < 1.; i += j) {
        ...
        s(p.yyz, p.yzz) s(p.zyz, p.zzz) s(p.zyy, p.zzy) s(p.yyy, p.yzy) s(p.yyy, p.zyy) s(p.yzy, p.zzy) s(p.yyz, p.zyz) s(p.yzz, p.zzz) s(p.zzz, p.zzy) s(p.zyz, p.zyy) s(p.yzz, p.yzy) s(p.yyz, p.yyy) }

    vec3 n1, q, r2 = iResolution, d = normalize(vec3((C * 2. - r2.xy) / r2.y, 1));
    for(float i = 0., a, s, e, g = 0.; ++i < 70.; O.xyz += mix(vec3(1), H(g * .1), .8) * 1. / e / 8e3) {
        n1 = g * d;
        ...
        for(int i = 0; i++ < 8;) {
            ...
            n1 = abs(n1) * e -
                vec3(5. + sin(iTime * .3 + .5 * sin(iTime * .3)) * 3., 120, 8. + cos(iTime * .5) * 5.) * dot(c.y, n1.x);
        }
        g += e = length(n1.xy) / s;
    }
