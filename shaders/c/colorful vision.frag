...
vec4 points(int i) {
  ...
  const vec4 POINTS[] = vec4[](normalize(vec4(cos(-a), sin(-a), cos(-a), sin(-a))), normalize(vec4(cos(-a), sin(-a), 1, 0)), normalize(vec4(cos(-a), sin(-a), cos(a), sin(a))), normalize(vec4(1, 0, cos(-a), sin(-a))), normalize(vec4(1, 0, 1, 0)), normalize(vec4(1, 0, cos(a), sin(a))), normalize(vec4(cos(a), sin(a), cos(-a), sin(-a))), normalize(vec4(cos(a), sin(a), 1, 0)), normalize(vec4(cos(a), sin(a), cos(a), sin(a))));
  return POINTS[i];
}
...
void mainImage(out vec4 O, vec2 C) {
  ...
  float r, dd, d = 9.;
  vec2 U = C;
  vec3 q2, P, R2 = iResolution, D = normalize(vec3((U - R2.xy / 2.) / R2.y, -1)), p = vec3(0, 0, 4), C2 = 3. * cos(.3 * t + vec3(0, 11, 0));
  ...
  while(R2.z++ < 120. && d > .01) {
    ...
    int i;
    while(i++ < 5) {
      q2 = P, q2.x -= .7, q2 = abs(q2) - 1.;

      i < 4 ? cor = cor.zxyw : O3;

      dd = min(L(q2.xyz), min(L(q2.yzx), L(q2.zxy))), d = d > dd ? O3 -= cor, dd : d, P.xy *= rot(1.26);
    }
  ...
  if(alert)
    col.r = 1.0; // Check nothing has gone wrong.

  vec3 n1, q, r2 = iResolution, d2 = normalize(vec3((C * 2. - r2.xy) / r2.y, 1));
  for(float i = 0., a, s, e, g = 0.; ++i < 110.; O.xyz += mix(vec3(1), H(g * .1), sin(.8)) * 1. / e / 8e3) {
    ...
    for(int i = 0; i++ < 8;) {
      ...
      n1 = abs(n1) * e -
        vec3(25. + cos(floor(abs(iTime) * exp(n1.x)) * .23 + .5 * sin(iTime * .3)) * 3., 100., 20. + dot(cos(iTime * 2.5), tan(iTime * 0.5))) * col;
    }
