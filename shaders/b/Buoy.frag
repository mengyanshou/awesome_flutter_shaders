// --- Migrate Log ---
// 添加 common_header 并声明 iChannel0；替换 textureLod(iChannel0, ...) 为 SG_TEX0/SG_TEXELFETCH0 以兼容 SkSL/Impeller（最小修改）
// --- Migrate Log (EN) ---
// Insert common_header and declare iChannel0; replace textureLod(iChannel0, ...) with SG_TEX0/SG_TEXELFETCH0 to be SkSL/Impeller compatible (minimal changes)
...
//	vec3 f2 = f*f; f = f*f2*(10.0-15.0*f+6.0*f2);
	...
	vec4 rg = texture(iChannel0, (uv+f.xy+0.5)/256.0);
...
//	vec3 f2 = f*f; f = f*f2*(10.0-15.0*f+6.0*f2);
	...
	vec4 rg = mix( mix(
				texture(iChannel0, (uv+0.5)/256.0),
				texture(iChannel0, (uv+vec2(1,0)+0.5)/256.0),
				f.x ),
			  mix(
				texture(iChannel0, (uv+vec2(0,1)+0.5)/256.0),
				texture(iChannel0, (uv+1.5)/256.0),
				f.x ),
				f.y );
...
//	vec3 f2 = f*f; f = f*f2*(10.0-15.0*f+6.0*f2);
	...
	vec2 uv = p.xy + f.xy;
	return texture(iChannel0, (uv+0.5)/256.0);
}

vec4 Noise( in ivec2 x )
{
	return texture(iChannel0, (vec2(x) + 0.5) / 256.0);
}

vec2 Noise( in ivec3 x )
{
	vec2 uv = vec2(x.xy)+vec2(37.0,17.0)*float(x.z);
	return texture(iChannel0, (uv + 0.5) / 256.0).xz;
}
