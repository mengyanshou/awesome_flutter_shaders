...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // if game mode (not game over), play game
        ...
        // ball
		//-------------------
        float dis = 0.01 * gameSpeed * (iTime * 60.0);
        ...
        for(int j = 0; j < 3; j++) {
            ...
            for(int j = 0; j < 3; j++) for(int i = 0; i < 3; i++) {
                    ...
                    if(id.x >= 0 && id.x < 13 && id.y >= 0 && id.y < 12) {
                        ...
                        if(brickHere > 0.5) {
                            vec2 ce = vec2(-1.0 + float(id.x) * brickW + 0.5 * brickW, 1.0 - float(id.y) * brickH - 0.5 * brickH);
            ...
            // did hit paddle
                ...
                balPosVel.zw = reflect(balPosVel.zw, nor);
                // borders bounce back
                if(balPosVel.x > (paddlePos + paddleSize * 0.5))
                    balPosVel.z = abs(balPosVel.z);
                else if(balPosVel.x < (paddlePos - paddleSize * 0.5))
                    balPosVel.z = -abs(balPosVel.z);
