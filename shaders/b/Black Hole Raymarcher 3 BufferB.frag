...
float interpolate(vec3 position) {
    ...
    vec4 lerpXY = vec4(rand(quantPos + vec3(0.0, 0.0, 0.0)), rand(quantPos + vec3(1.0, 0.0, 0.0)), rand(quantPos + vec3(1.0, 1.0, 0.0)), rand(quantPos + vec3(0.0, 1.0, 0.0)));

    vec4 lerpXYZ = vec4(rand(quantPos + vec3(0.0, 0.0, 1.0)), rand(quantPos + vec3(1.0, 0.0, 1.0)), rand(quantPos + vec3(1.0, 1.0, 1.0)), rand(quantPos + vec3(0.0, 1.0, 1.0)));
...
vec2 distField(vec3 position, vec3 origin) {
    ...
    float spiral;
    float occ;
    spiral = octave(vec3(dist, 50.0 * (1.0 + sin(angle)), 1.0 * distance(origin.z + 3.0 * iTime, position.z)), detail, density);//3d noise function
...
vec3 rayCast(vec2 rayAxis) {
    ...
    for(int i = 0; i <= steps; i++) {
       ...
       //Deflect the ray for gravity
        rayVel += gravDis * pow(stepSize, 2.4) * vec3((rayDefl.x) * (1.0 / pow(rayDist, 4.0)), (rayDefl.y) * (1.0 / pow(rayDist, 4.0)), (rayDefl.z) * (1.0 / pow(rayDist, 4.0)));
    ...
    if(rayDist >= holeRadius * 3.0) {
        rayVol += starField(rayVel) / (occ);
    }
    return vec3(rayVol, clamp(rayVol * colShift, 0.0, 1.0), rayVol);
}
